...
export async function migrateContentDatabase(): Promise<void> {
  const db = sql();
  ...
  `;
  await db`insert into portfolio_content_state (singleton, active_version_id) values (true, null) on conflict (singleton) do nothing`;
}
